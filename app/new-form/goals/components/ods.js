...
export default function ODS({ onPriorClick, onNextClick }) {
  ...
  function handleODSClick(odsItem) {
    if (ods.findIndex((item) => item.id === odsItem.id) >= 0) {
      removeODS(odsItem);
    } else {
      addODS(odsItem);
    }
  ...
  return (
    <Card
      onPriorClick={() => onPriorClick("ods")}
      onNextClick={handleNextClick}
      nextActive={ods.length > 0}
    >
        ...
        <div className="mt-10 flex flex-wrap gap-1 px-28">
          {odsList.map((odsItem) => (
            <ODSOption
              key={`ods-${odsItem.id}`}
              icon={odsItem.icon}
              selected={ods.findIndex(item => item.id === odsItem.id) >= 0}
              onClick={() => handleODSClick(odsItem)}
